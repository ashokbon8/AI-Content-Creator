...
const contactMethods: ContactMethod[] = [
  {
    icon: <Mail className="w-6 h-6 text-[#0d9488]" />, // Set icon color
    title: "Email Support",
  ...
  {
    icon: <Phone className="w-6 h-6 text-[#0d9488]" />, // Set icon color
    title: "Phone Support",
  ...
  <div className={`rounded-xl overflow-hidden transition-all duration-300 bg-black hover:bg-gray-800 shadow-md`}>
    <button onClick={onToggle} className="w-full flex items-center justify-between p-6 text-left font-medium">
      <span className="flex items-center gap-3">
        <span className="text-2xl text-[#0d9488]">{faq.icon}</span> {/* Set icon color */}
    ...
    <div className={`p-6 rounded-xl bg-black shadow-md`}>
      <div className="flex flex-wrap gap-2 mb-4">
        {methods.map((method, index) => (
          <button
            key={index}
            className={`px-4 py-2 font-semibold rounded-lg transition-all duration-300 ${selectedMethodIndex === index ? 'bg-[#0d9488] text-white' : 'text-gray-600 hover:bg-gray-100'}`}
      ...
      <div className="flex items-start space-x-4">
        <div className='text-[#0d9488]'>{methods[selectedMethodIndex].icon}</div> {/* Set icon color */}
        <div>
          <h3 className="font-semibold mb-1 text-[#0d9488]">{methods[selectedMethodIndex].title}</h3> {/* Set title color */}
          ...
          <div className="flex flex-wrap justify-center gap-2 mb-8">
            {categories.map((category) => (
              <button
                ...
                className={`px-4 py-2 rounded-lg font-medium transition-all duration-300 ${selectedCategory === category ? 'bg-[#0d9488] text-white' : 'bg-black text-gray-600 hover:bg-gray-100'}`}
              >
